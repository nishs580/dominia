...
- **AllianceScreen wire "▌ LIVE" label**: owner decision 2026-07-22 — kept as-is; wiring a real realtime subscription (Ably, like chat) is a deferred backend task. Do not re-flag the label as dishonest until the realtime feed is built.
- **AllianceScreen roster weekly-steps**: the per-member weekly step/contribution leaderboard is not in the detail payload (backend returns level/avatar/recruit_streak/joined_at only). The roster now shows real level + avatar; a true weekly-activity leaderboard needs a new backend field. Not a defect.
- **ProfileScreen two-red decision** (avatar edit badge + Delete-account row both Claim Red): owner decision 2026-07-22 — the avatar edit/add badge keeps its red affordance, and the Delete-account row is the one destructive red. All other reds were corrected (rank title/XP fill/wallet/sign-out → neutral; alliance name → Alliance Green). Do not re-flag the avatar badge or delete row as a One-Claim violation.
