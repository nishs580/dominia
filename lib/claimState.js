...
const CONTINUATION_FIELDS = ['strideM', 'strideSessions'];
...
export async function rehydrateFromStorage() {
  try {
    ...
    const wasActive = claimState.active === true;
    console.log(`[claimState] rehydrate: wasActive=${wasActive}, snapshotActive=${parsed.active}`);
    if (wasActive) {
      for (const key of CONTINUATION_FIELDS) {
        if (claimState[key] == null && parsed[key] != null) {
          claimState[key] = parsed[key];
        }
      }
    } else {
    ...
    console.warn('[claimState] rehydrate fail', e?.message);
    return false;
  }
}
