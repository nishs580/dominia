...
export async function postMessage({ clerkGetToken, roomId, content }) {
  return performChatRequest({
    clerkGetToken,
    url: `${BACKEND_URL}/chat/rooms/${roomId}/messages`,
    method: 'POST',
    body: JSON.stringify({ content }),
  });
}

export async function patchAllianceChatPushEnabled({
  clerkGetToken,
  enabled,
}) {
  return performChatRequest({
    clerkGetToken,
    url: `${BACKEND_URL}/me/alliance-chat-push-enabled`,
    method: 'PATCH',
    body: JSON.stringify({ alliance_chat_push_enabled: !!enabled }),
  });
}
