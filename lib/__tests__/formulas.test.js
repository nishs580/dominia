...
describe('validation: dev level', () => {
  test('valid dev levels 0–4 do not throw via getCumulativeDevCost', () => {
    for (const lvl of [0, 1, 2, 3, 4]) {
      expect(() => F.getCumulativeDevCost(lvl)).not.toThrow();
    }
  });

  test('invalid dev levels throw', () => {
    expect(() => F.getCumulativeDevCost(-1)).toThrow(/Invalid dev level/);
    expect(() => F.getCumulativeDevCost(5)).toThrow(/Invalid dev level/);
    expect(() => F.getCumulativeDevCost('2')).toThrow(/Invalid dev level/);
    expect(() => F.getCumulativeDevCost(null)).toThrow(/Invalid dev level/);
  });
});
...
describe('calcClaimXp', () => {
  test('returns 40 for Small', () => {
    expect(F.calcClaimXp('Small')).toBe(40);
  });
  test('returns 80 for Medium', () => {
    expect(F.calcClaimXp('Medium')).toBe(80);
  });
  test('returns 200 for Large', () => {
    expect(F.calcClaimXp('Large')).toBe(200);
  });
  test('returns 500 for Epic', () => {
    expect(F.calcClaimXp('Epic')).toBe(500);
  });
  test('throws on invalid tier', () => {
    expect(() => F.calcClaimXp('Tiny')).toThrow();
  });
  test('throws on lowercase tier (strict casing)', () => {
    expect(() => F.calcClaimXp('small')).toThrow();
  });
});

describe('calcContestWinXp', () => {
  test('returns 300 for Small', () => {
    expect(F.calcContestWinXp('Small')).toBe(300);
  });
  test('returns 350 for Medium', () => {
    expect(F.calcContestWinXp('Medium')).toBe(350);
  });
  test('returns 450 for Large', () => {
    expect(F.calcContestWinXp('Large')).toBe(450);
  });
  test('returns 600 for Epic', () => {
    expect(F.calcContestWinXp('Epic')).toBe(600);
  });
  test('throws on invalid tier', () => {
    expect(() => F.calcContestWinXp('Massive')).toThrow();
  });
});

describe('CLAIM_XP and CONTEST_WIN_XP constants', () => {
  test('CLAIM_XP is frozen', () => {
    expect(Object.isFrozen(F.CLAIM_XP)).toBe(true);
  });
  test('CONTEST_WIN_XP is frozen', () => {
    expect(Object.isFrozen(F.CONTEST_WIN_XP)).toBe(true);
  });
});
...
describe('calcDailyInfluenceFixedPoint', () => {
  ...
  test('peak output (upkeep removed): Epic D4 R5: 25 × 1.75 × 3.0 × 10 = 1312.5 → 1313', () => {
    // Formerly halved by unpaid upkeep; upkeep no longer exists (§5.3.1 retired).
    expect(F.calcDailyInfluenceFixedPoint({
...
describe('calcMedalPower', () => {
  test('non-array / empty produce zero', () => {
    expect(F.calcMedalPower(null)).toBe(0);
    expect(F.calcMedalPower(undefined)).toBe(0);
    expect(F.calcMedalPower([])).toBe(0);
  });

  test('tiered medal scores its highest earned tier; unearned scores zero', () => {
    expect(F.calcMedalPower([{ type: 'tiered', currentTier: 'bronze' }])).toBe(50);
    expect(F.calcMedalPower([{ type: 'tiered', currentTier: 'silver' }])).toBe(100);
    expect(F.calcMedalPower([{ type: 'tiered', currentTier: 'gold' }])).toBe(200);
    expect(F.calcMedalPower([{ type: 'tiered', currentTier: 'claim' }])).toBe(400);
    expect(F.calcMedalPower([{ type: 'tiered', currentTier: null }])).toBe(0);
  });

  test('singular x N: 50 per count, capped at 500', () => {
    expect(F.calcMedalPower([{ type: 'singular_count', count: 3 }])).toBe(150);
    expect(F.calcMedalPower([{ type: 'singular_count', count: 10 }])).toBe(500);
    expect(F.calcMedalPower([{ type: 'singular_count', count: 25 }])).toBe(500);
    expect(F.calcMedalPower([{ type: 'singular_count', count: 0 }])).toBe(0);
  });

  test('singular one-off: 500 when earned, else 0', () => {
    expect(F.calcMedalPower([{ type: 'singular_oneoff', earned: true }])).toBe(500);
    expect(F.calcMedalPower([{ type: 'singular_oneoff', earned: false }])).toBe(0);
  });

  test('mixed set sums correctly', () => {
    expect(F.calcMedalPower([
      { type: 'tiered', currentTier: 'gold' },     // 200
      { type: 'tiered', currentTier: 'bronze' },   // 50
      { type: 'singular_count', count: 3 },        // 150
      { type: 'singular_count', count: 20 },       // 500 (capped)
      { type: 'singular_oneoff', earned: true },   // 500
    ])).toBe(1400);
  });

  test('full completion caps near 6.7k', () => {
    const full = [];
    for (let i = 0; i < 13; i++) full.push({ type: 'tiered', currentTier: 'claim' });
    full.push({ type: 'singular_count', count: 50 });
    full.push({ type: 'singular_count', count: 50 });
    full.push({ type: 'singular_oneoff', earned: true });
    expect(F.calcMedalPower(full)).toBe(6700);
  });
});
...
// (calcWeeklyUpkeep removed — territory upkeep retired with Territory
// Development ship; mechanics §5.3.1. No replacement mechanic.)
