...
let getAblyToken;
let postMessage;
let patchAllianceChatPushEnabled;
...
function loadChatApi() {
  ...
  source +=
    '\nreturn { getRooms, getMessages, patchReadState, getAblyToken, postMessage, patchAllianceChatPushEnabled };';
...
describe('postMessage — POST + happy path', () => {
  test('201 returns envelope with sender enrichment', async () => {
    const envelope = {
      message: {
        id: 'm-server',
        room_id: 'r-1',
    ...
    mockFetchResponse(201, envelope);
...
describe('postMessage — error discriminants', () => {
  test('422 message_filtered -> code preserved', async () => {
    mockFetchResponse(422, { error: 'message_filtered' });
  ...
  test('403 chat_muted -> muted_until preserved in context', async () => {
    mockFetchResponse(403, {
      error: 'chat_muted',
      muted_until: '2026-06-21T13:00:00.000Z',
    });
    ...
    expect(result).toEqual({
      ...
      status: 403,
      code: 'chat_muted',
      context: { muted_until: '2026-06-21T13:00:00.000Z' },
    });
  });

  test('429 rate_limited -> retry_after_seconds preserved', async () => {
    mockFetchResponse(429, { error: 'rate_limited', retry_after_seconds: 25 });
    ...
    expect(result).toEqual({
      ...
      status: 429,
      code: 'rate_limited',
      context: { retry_after_seconds: 25 },
    });
  });

  test('403 room_access_forbidden -> room_id preserved', async () => {
    mockFetchResponse(403, {
      error: 'room_access_forbidden',
      room_id: 'r-1',
    });
  ...
  test('400 invalid_content -> code preserved', async () => {
    mockFetchResponse(400, { error: 'invalid_content' });
...
describe('patchAllianceChatPushEnabled — toggle wire shape', () => {
  test('PATCH /me/alliance-chat-push-enabled with bool body', async () => {
    mockFetchResponse(200, {
      ok: true,
      alliance_chat_push_enabled: false,
    });
    ...
    const [url, options] = global.fetch.mock.calls[0];
    expect(url).toBe(`${BACKEND_URL}/me/alliance-chat-push-enabled`);
    expect(options.method).toBe('PATCH');
    expect(JSON.parse(options.body)).toEqual({
      alliance_chat_push_enabled: false,
    });
  ...
  test('enabled coerces to boolean', async () => {
    mockFetchResponse(200, { ok: true });

    await patchAllianceChatPushEnabled({ clerkGetToken, enabled: 1 });

    const [, options] = global.fetch.mock.calls[0];
    expect(JSON.parse(options.body).alliance_chat_push_enabled).toBe(true);
  });
});
