 ...
 *
 * @returns {Promise<{ok: true, data: { alliance_id: string }} | {ok: false, status: number, error: Object | string}>}
 *
 * Never throws — caller can rely on {ok} discriminant.
 */
export async function foundAlliance({ clerkGetToken, fullName, shortName, hqTerritoryId }) {
  try {
    const token = await clerkGetToken();
    if (!token) {
      return { ok: false, status: 401, error: 'no_token' };
    ...
    const data = await res.json();
    return { ok: true, data: { alliance_id: data.alliance_id } };
  } catch (err) {
