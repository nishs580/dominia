...
async function _requestWith401Retry(clerkGetToken, fetchFn) {
  async function attempt() {
    ...
    console.log('[activityLogApi] non-2xx', status, errBody);
  ...
  try {
    ...
    console.log('[activityLogApi] network error', err?.message ?? err);
    return { ok: false, status: 0, error: 'network_error' };
  }
}

function _buildActivityLogUrl({ limit, cursor }) {
  const params = new URLSearchParams();
  if (limit !== undefined) params.set('limit', String(limit));
  if (cursor !== undefined) params.set('cursor', String(cursor));
  const qs = params.toString();
  return `${BACKEND_URL}/me/activity-log${qs ? `?${qs}` : ''}`;
}

/**
 * Fetch paginated activity log via GET /me/activity-log.
 *
 * @param {Object} opts
 ...
 */
export async function getActivityLog({ clerkGetToken, limit, cursor }) {
  const url = _buildActivityLogUrl({ limit, cursor });
...
/**
 * Mark activity log read via PATCH /me/activity-log/read.
 *
 * @param {Object} opts
 * @param {Function} opts.clerkGetToken — async function returning Clerk JWT
 * @param {string} [opts.readUpTo] — ISO timestamp; omitted → server uses NOW()
 *
 * @returns {Promise<{ok: true, data: { activity_log_last_read_at }} | {ok: false, status: number, error: string}>}
 *
 * Never throws — caller can rely on {ok} discriminant.
 */
export async function markActivityLogRead({ clerkGetToken, readUpTo }) {
  const body = {};
  if (readUpTo !== undefined) body.readUpTo = readUpTo;

  return _requestWith401Retry(clerkGetToken, (token) =>
    fetch(`${BACKEND_URL}/me/activity-log/read`, {
      method: 'PATCH',
      headers: {
