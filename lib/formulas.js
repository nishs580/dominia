...
const XP_PER_CLAIM = Object.freeze({
  Small: 40,
  Medium: 80,
  Large: 200,
  Epic: 500,
...
const XP_PER_CONTEST_WIN = Object.freeze({
  Small: 300,
  Medium: 350,
  Large: 450,
  Epic: 600,
...
// Weekly Stone upkeep at each development level
const WEEKLY_UPKEEP_STONE = Object.freeze({
  0: 0, 1: 0, 2: 6, 3: 9, 4: 12,
});

// Penalty when upkeep is overdue
const UNPAID_UPKEEP_INFLUENCE_FACTOR = 0.5;
...
function calcClaimXp(tier) { _assertTier(tier); return XP_PER_CLAIM[tier]; }
function calcContestWinXp(tier) { _assertTier(tier); return XP_PER_CONTEST_WIN[tier]; }
 ...
 * @param {boolean} territory.upkeepOverdue — true if D2+ and weekly Stone unpaid
 * @returns {number} integer Influence × 10
 */
function calcDailyInfluenceFixedPoint(territory) {
  const { tier, developmentLevel, legacyRank, upkeepOverdue = false } = territory;
  _assertTier(tier);
  ...
  const upkeepFactor = upkeepOverdue ? UNPAID_UPKEEP_INFLUENCE_FACTOR : 1.0;

  const raw = base * rankMult * devMult * upkeepFactor;
...
/**
 * Returns the weekly Stone upkeep for a territory at a given dev level.
 * Per v6.10 §5.3.1: upkeep is each territory holder's individual responsibility.
 */
function calcWeeklyUpkeep(developmentLevel) {
  _assertDevLevel(developmentLevel);
  return WEEKLY_UPKEEP_STONE[developmentLevel];
}
...
module.exports = {
  // constants (read-only)
  TIERS, DEV_LEVELS, LEGACY_RANKS, CHALLENGE_TIERS, STREAK_TIERS,
  BONUS_PRODUCT_CAP,
  LEVEL_XP_FLOORS, LEVEL_TITLES,
  XP_PER_CHALLENGE, XP_PER_CLAIM, XP_PER_CONTEST_WIN, XP_PER_DEFENCE_WIN,
  ...
  RESOURCE_EARN,
  CLAIM_GOLD_COST, CLAIM_GOLD_REWARD, CONTEST_IRON_COST,
  ...
  DEV_COST, WEEKLY_UPKEEP_STONE, UNPAID_UPKEEP_INFLUENCE_FACTOR,
  ...
  ARMOUR_COSTS,
  ACTIVITY_POWER_WEIGHTS, LEGACY_POWER_WEIGHTS,
  ...
  calcDevCost, getCumulativeDevCost, calcWeeklyUpkeep,
  ...
  calcLegacyPower, calcTotalPower, calcAlliancePower,
