 ...
 * Supports { username, has_onboarded }. Replaces the direct
...
/**
 * Live username-availability check via GET /me/username-available.
 * Returns { ok:true, data:{ available:boolean } } for valid names, or
 * { ok:false, status:400 } when the format is rejected server-side.
 *
 * @returns {Promise<{ok: true, data: { available: boolean }} | {ok: false, status: number, error: any}>}
 */
export async function checkUsernameAvailable({ clerkGetToken, username }) {
  try {
    ...
    const url = `${BACKEND_URL}/me/username-available?username=${encodeURIComponent(username)}`;
