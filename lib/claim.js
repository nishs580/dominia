...
export async function pushCalibrationSample(playerId, gpsDistM, stepsInWindow, currentSamples) {
  if (!playerId || !Number.isFinite(gpsDistM) || !Number.isFinite(stepsInWindow) || stepsInWindow <= 0) {
    return null;
  }
  const newSample = gpsDistM / stepsInWindow;
  // Sanity bounds — reject samples outside human walking stride range
  if (newSample < 0.4 || newSample > 1.1) {
    console.warn('[claim] rejected out-of-range stride sample:', newSample);
    return null;
  }
  const baseSamples = Array.isArray(currentSamples) ? currentSamples : [];
  const nextSamples = [...baseSamples, newSample].slice(-CALIBRATION_SAMPLE_CAP);
  const nextStride = nextSamples.reduce((a, b) => a + b, 0) / nextSamples.length;

  const { data, error } = await supabase
    .from('players')
    .update({
      stride_length_m: nextStride,
      stride_calibration_samples: nextSamples,
      stride_calibration_sessions: (baseSamples.length === 0 ? 1 : baseSamples.length + 1),
    })
    .eq('id', playerId)
    .select()
    .maybeSingle();
  ...
  if (error) {
    console.warn('[claim] pushCalibrationSample error:', error.message);
    return null;
  }
  return {
    strideM: data?.stride_length_m ?? nextStride,
    sessions: data?.stride_calibration_sessions ?? nextSamples.length,
    samples: nextSamples,
  };
}
