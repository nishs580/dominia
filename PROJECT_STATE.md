...
Last updated: June 13, 2026 (Sessions 62–66 — **Activity Slice 7 SHIPPED on `main`**: CC enforcement + §13 cap-factor backend (S62), mobile flushNow await (S63), claim endpoint backend-authority cutover (S64), contest backend-authority cutover (S65a), contest mobile cutover (S65b). **Device smoke still DEFERRED** (outside contest hours at session close) — Slice 7 not formally closed until verified. **Post-ship incident**: an S65a test `TRUNCATE … CASCADE` ran against PRODUCTION (single Supabase project, no separate test DB existed); recovered from the 12 Jun 06:19 UTC scheduled backup. S66 shipped **B-S65b-X-test-db-isolation** — backend tests now run against an isolated local Docker Postgres with a three-layer DATABASE_URL guard; backend test gate UNBLOCKED.)
...
| Test runner | Jest 29.7 + jest-expo (mobile, **451 tests** — 348 formulas + 63 activity.helpers + 40 contest/claim API+producer+helpers added S65b; 451/451 × 4 baseline clean) · backend: `node --test --import tsx` via `npm test`, which **auto-bootstraps an isolated local Docker Postgres** (S66) and runs the **full 518-test suite** sequentially (`--test-concurrency=1`, ~34–37s/run, 4-run zero-flake gate). The historic "79 pre-existing Supabase statement-timeout failures" workaround is obsolete — the full suite now runs clean against the isolated DB. `npx tsc --noEmit` is a mandatory pre-test gate (S65a). | ✓ Green (full suite, isolated DB) |
...
│   │   │   ├── contest-defend.{routes,service,queries}.ts ✓ // /defend endpoint LIVE; defender mobile UX still unbuilt (B-S65-X-defender-lifecycle)
...
| Defender flow | ○ Deferred | Backend `/defend` lifecycle live; mobile UI not built. **Next major feature slice: B-S65-X-defender-lifecycle** (accept UI + defender walk producer — likely reuses `lib/contestWalk.js` — + result-screen branches + "you are under attack" push). The `role: 'defender'` code path is already plumbed in S65b nav params. |
...
# Unit tests (451 tests across 5 suites; must run 4x zero-flake before any commit)
npm test
...
#   2. npm test                    # auto-bootstraps the test container, runs the FULL 518-test suite sequentially (~34-37s). Run 4x, zero-flake.
...
**Module sequence (locked): Progression ✅ → Activity ✅ (Slice 6 + S52a; Slice 7 SHIPPED S62–S65b, smoke pending) → Notifications ✅ (S53–S61b, CLOSED) → Contest Defender Lifecycle → Leaderboard → Realm.**

**S67 PRIORITIES (hard-ordered):**

1. **B-S65b-X-onboarding-hardening (scoped down — b/c/d only).** Small, contained, files `components/AuthGate.js` + `screens/OnboardingScreen.js`. Hardens the missing/mismatched-row cold-start path so it can never strand a user on "Loading session…" again.
   - (b) `OnboardingScreen` `resolvedPlayerId` useEffect (~line 158): add error handling, no-row handling, and a retry affordance. Real bug — any transient failure currently strands the user forever.
   - (c) `AuthGate` already has `players.id` at query time → pass it via `nav.replace('Onboarding', { playerId })`, removing the duplicate Supabase lookup in OnboardingScreen.
   - (d) Clerk-ID drift detection: explicit detection + clear UI for "Clerk userId does not match any player row" — sign-out + retry CTA, not stuck loading.
   - **SKIPPED:** (a) AuthGate missing-row fallback — DB-wipe-specific; the restored DB + test isolation make this defensive net redundant.

2. **S65b smoke verification on nish_s** — Activity Slice 7 closure (long-deferred). Must run during 05:00–22:59 local (attacker home_timezone contest gate). Full happy path + the three inline-error spot-checks. Can run in parallel with (1). Until verified, Slice 7 is NOT closed.

3. **B-S65-X-defender-lifecycle** — the next MAJOR feature slice, only after (1)+(2). POST `/defend` endpoint + defender accept UI + defender walk producer (likely reuses/refactors `lib/contestWalk.js`) + defender result-screen branches + "you are under attack" push. Activates the `role: 'defender'` code path already plumbed in S65b nav params. Est. 6–8 hours, 12–18 decision locks.

**After defender lifecycle + smoke clean → Leaderboard module** (alliance + global rankings; Redis Sorted Set; gated on Activity Slice 7 beta-data accumulation). **Then Realm module.**

**Slice 8 — kcal Phase 2 (mobile + iOS HealthKit)** remains queued behind Slice 7 smoke: HC ActiveCaloriesBurned read on Android + first iOS slice. Backend has accepted `kcal` since S49; mobile producer omits it per S51 scope.

**Quick alternative picks if blocked:** (a) `total_xp` stale-response one-liner on milestone days (~15 min); (b) GET /alliances?city=X endpoint (~30 min); (c) reconquest +400 XP writer — §7.7, schema ready, hooks into contest-walk attacker_won; **must also call `emitFirstEarnPush({source:'reconquest'})` — composer entry pre-staged from S60** (~half-day); (d) test-row cleanup (B-S66-X-test-row-cleanup-deferred) now safe to run post-isolation; (e) the B-S62/B-S65a/B-S66 backlog carry-forwards (rename sweeps, casing sweep, drift check).
...
- **Contest Defender Lifecycle slice (B-S65-X-defender-lifecycle)** — NEXT major feature slice after onboarding-hardening + Slice 7 smoke. POST `/defend` is already live backend-side; this wires the defender accept UI, defender walk producer (reuses `lib/contestWalk.js`), result-screen branches, and the "you are under attack" push. The `role: 'defender'` path is pre-plumbed in S65b nav params.
...
- **Leaderboard module** — Redis Sorted Set reads, ZADD on contest resolution. Gated on Defender Lifecycle + Activity Slice 7 smoke + beta-data accumulation.
