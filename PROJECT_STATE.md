...
Last updated: June 16, 2026 (Sessions 68–74 — **Defender Lifecycle Module CODE-COMPLETE on `main`, status SHIPPED-PENDING-SMOKE**. Backend sub-slice 1 shipped across S69–S72 in 5 atomic groups: G1 error normalization (`6587a87`), G2 contest alliance-id population (`906f8cc`), G3 defender auth widening + cutoff (`adacf10`), G4 defender push fan-out (`33d25b5`), G5 `GET /contests/:id/defend-preview` endpoint (`7761384`). Mobile sub-slice 2 shipped S73–S74 in 4 atomic groups: M1 defender API client (`eaa9604`), M2 DefenderAcceptScreen (`0111011`), M3 push deep-link params + extractParams infra (`3528881`), M4 role plumbing + ContestResultScreen defender branches (`a737695`). **Device smoke DEFERRED to S75** — `PROJECT_STATE.md` updated now under explicit user override of the "after a full module ships" rule. Smoke covers happy path (Alyona attacks `nish_s`, defender_notify push → DefenderAcceptScreen → toggle stone → ACCEPT → ActiveClaim role=defender → defender_won → ContestResultScreen with STONE earned-beat) + already-defended terminal + past-cutoff terminal + preview-retry + 8 submit-error codes. Activity Slice 7 smoke ALSO still pending (carried from S66). Backend test baseline: 565/565 zero-flake at `7761384`; mobile: 488/488 zero-flake at `a737695` (up from 471 at S74 start; +17 in new `lib/notifications/__tests__/route.test.js`).)
...
| Test runner | Jest 29.7 + jest-expo (mobile, **488 tests** — 348 formulas + 63 activity.helpers + 40 contest/claim API+producer+helpers + 17 push-route table tests added S74; 488/488 × 4 baseline clean at HEAD `a737695`) · backend: `node --test --import tsx` via `npm test`, which **auto-bootstraps an isolated local Docker Postgres** (S66) and runs the **full 565-test suite** sequentially (`--test-concurrency=1`, ~34–37s/run, 4-run zero-flake gate at HEAD `7761384`). The historic "79 pre-existing Supabase statement-timeout failures" workaround is obsolete — the full suite now runs clean against the isolated DB. `npx tsc --noEmit` is a mandatory pre-test gate (S65a). | ✓ Green (full suite, isolated DB) |
...
│   ├── app.ts                       ✓ Fastify instance factory. Registers: health, player, me (incl. challenge-complete), territory, contestDefendRoutes (direct), contestWalkRoutes (direct), **activity routes (S49)**, debug routes. Calls bootstrapStreakRolloverJobs + bootstrapStreakBreakWarningJobs after Fastify ready.
...
# Unit tests (488 tests across 6 suites; must run 4x zero-flake before any commit)
npm test
...
#   2. npm test                    # auto-bootstraps the test container, runs the FULL 565-test suite sequentially (~34-37s). Run 4x, zero-flake.
...
**Module sequence (locked): Progression ✅ → Activity ✅ (Slice 6 + S52a; Slice 7 SHIPPED S62–S65b, smoke pending) → Notifications ✅ (S53–S61b, CLOSED) → Defender Lifecycle ✅ (S68–S74 code-complete, smoke pending) → Leaderboard → Realm.**

**S75 PRIORITIES (hard-ordered):**

1. **Defender lifecycle device smoke — sub-slice 2 closure gate.** Must run within contest hours (05:00–22:59 local). Setup: Metro running on `nish_s` (OnePlus 12, HEAD `a737695`); Alyona's EAS preview build still has the older attacker flow (no rebuild needed since Alyona only plays attacker for this smoke); `nish_s` owns at least one Alyona-reachable territory; FCM working. 
   - **Happy path:** Alyona attacks `nish_s` → defender_notify push fires → tap (foreground + background + killed-state matrix) → DefenderAcceptScreen renders 16-field preview → toggle stone/no-stone → ACCEPT → POST /defend 200 → ActiveClaim with `role='defender'` → walk → /walk resolves `defender_won` → ContestResultScreen renders `defence_won` with `earnedBeat` showing STONE not IRON.
   - **Alternate flows:** already-defended terminal (third alliance member accepts first) · past-cutoff terminal (Alyona walks past 75% before nish_s opens preview) · preview-fetch error + retry · submit-error mapping for each of the 8 backend defend error codes where reachable.
   - **Pass criteria:** no console errors, no silent navigation failures, no STATE_CONFIG default fallbacks. Any regression → `B-S75-X-...` carry-forward; do NOT close the module until clean.

2. **Activity Slice 7 device smoke (long-deferred from S66).** Same `nish_s` device, same contest hours. Full happy path + the three inline-error spot-checks (insufficient_iron / level_too_low / outside_contest_hours). Until verified, Slice 7 is also NOT closed.

3. **Roadmap close-outs.** Mark DEFENDER_ROADMAP.md sub-slice 2 ✓ closed; mark Activity Slice 7 closed if (2) passes. Update `PROJECT_STATE.md` to remove SHIPPED-PENDING-SMOKE status (or, if smoke fails, refine carry-forwards).

4. **Carry-forward closure assessment.** Walk B-S73-X-* and B-S74-X-* lists; close anything that's no longer relevant after smoke; keep the open ones (notably `contest-result-push-deeplink-params`, `defender-threshold-display-policy`, `first-defence-win-plumbing`).

5. **B-S65b-X-onboarding-hardening (b/c/d only, scoped down).** Carried forward from S67 priorities — still applicable. Small, contained: `components/AuthGate.js` + `screens/OnboardingScreen.js`. (b) error/no-row/retry on `OnboardingScreen` `resolvedPlayerId` useEffect; (c) AuthGate passes `playerId` via nav-param to remove duplicate Supabase lookup; (d) Clerk-ID drift detection with clear sign-out CTA. SKIPPED: (a) AuthGate missing-row fallback (restored DB + S66 isolation made it redundant).

**Next-module kickoff (post-S75 closure):**

- **Leaderboard module** — alliance + global rankings; Redis Sorted Set ZADD on contest resolution + ZREVRANGE on read. Previously gated on defender lifecycle + Activity Slice 7 smoke + beta-data accumulation; both gates resolve at S75 if smokes pass.
...
- **Realm module** — further out per locked sequence.

**Quick alternative picks if blocked:** (a) `total_xp` stale-response one-liner on milestone days (~15 min); (b) GET /alliances?city=X endpoint (~30 min); (c) reconquest +400 XP writer (§7.7, schema ready; **must also call `emitFirstEarnPush({source:'reconquest'})` — composer entry pre-staged from S60**, ~half-day); (d) test-row cleanup (B-S66-X-test-row-cleanup-deferred) now safe to run post-isolation; (e) any of the B-S62/B-S65a/B-S66/B-S73/B-S74 backlog carry-forwards (rename sweeps, casing sweep, drift check, shared-fetch helper).
...
- **Leaderboard module** — Redis Sorted Set reads, ZADD on contest resolution. Previously gated on Defender Lifecycle + Activity Slice 7 smoke + beta-data accumulation. Code-side gates now resolved (defender module shipped-pending-smoke); both smokes target S75 for closure.
