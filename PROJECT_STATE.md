...
| War Room screen | ✓ Live data | **(REWORKED Jul 7–8 — abilities now functional, pending rebuild + on-device)** 5 abilities (Unified Front REMOVED). Panel driven by `GET /alliances/:id/abilities` (server-authoritative costs, window chips SAT–SUN / MON–FRI 1×/WEEK). Button lifecycle: **ACTIVATE → ACTIVATED** (alliance-green, while the buff runs) **→ 8h cooldown countdown** (only after the buff ends) **→ ACTIVATE** again once clear; Supply Line shows used-this-week / resets-Monday. ACTIVATE → `POST /alliances/:id/abilities/:ability/activate` (no client cost; **Founder + Marshal** via server `can_manage`), replaces the old `deduct_alliance_morale` RPC / `spendAllianceMorale`. Error alerts map server reject codes (morale/active/cooldown/week/window). Combat abilities' Sat/Sun window is **always server-enforced** (buttons sit disabled on weekdays). **Attack Day countdown card is REAL** — counts to next Sat/Sun 05:00 (labelled TODAY/SATURDAY/SUNDAY), flips to a LIVE state (claim accent, counts to 23:00 close) during the weekend window; mirrors the 05:00–23:00 backend contest hours. Countdown units localised (en D/H/M, ru Д/Ч/М). |
...
| **Ability day-windows ALWAYS enforced (not gated); cooldown starts AFTER the buff duration** | (Jul 7, corrected Jul 8 after smoke test) The 4 combat abilities are weekend-only (Sat/Sun); Supply Line is weekday-only (Mon–Fri), once per Monday-anchored calendar week (leader home tz). The day-window is always enforced — the earlier version gated it behind `ATTACK_DAY_GATE` and combat abilities activated on a Thursday. `ATTACK_DAY_GATE` governs only when contests may start; ability windows are independent. The 8h cooldown starts when the buff EXPIRES (not at activation), so the lock = duration + 8h; the button reads ACTIVATED during the buff, then a cooldown countdown. All three (window/cooldown/week) are DERIVED from the latest `activated_at`+`expires_at` — no state column to go stale. |
