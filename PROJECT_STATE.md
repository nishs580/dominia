...
| ORM | Prisma 7.8 (`@prisma/client` + adapter-pg, 15 models, schema introspected from live Supabase) | ✓ Live (singleton + adapter-pg) |
...
│   │   │   ├── velocity.ts              ✓ // (S46/S47) MAX_PLAUSIBLE_KMH=25, MAX_PLAUSIBLE_MS (derived),
...
│
├── prisma/schema.prisma             ✓ Introspected from live Supabase — 15 models (added `activity_samples` in S48). PostGIS as `Unsupported("geometry")`.
